...
local webhookurl = 'https://discordapp.com/api/webhooks/XXXXXXXXX/XXXCHANGEMEXXX' -- Insira seu Webhook dentro dos apóstrofos.
local modeloparaLSC = false; -- Se você precisa que gere num formato semelhante ao necessário para colocar na Los Santos, deixe true. 


RegisterServerEvent('likzao:hashenvio')
AddEventHandler('likzao:hashenvio', function(hash,Model)
      if modeloparaLSC then 
	      SendWebhookMessage("{modelHash = "..hash..", modelName = '"..Model.."'")      
      else
          local fields = {}
